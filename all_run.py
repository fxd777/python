...
logging.basicConfig(level=logging.DEBUG,  #critical error warning info debug 日志事件级别  严重信息>>>详细信息
          filename=r'E:\pychon-code\a_web_auto_scirpt\log\ctrip.txt', #输出日志的保存路径
          filemode='a',  # a表示追加的模式打开文件
          format='%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s')  #保存的格式

# 选取要执行的脚本文件
def discover_all_cases():
    discover=unittest.defaultTestLoader.discover(r"E:\pychon-code\a_web_auto_scirpt\po\test_cases\\",pattern="test*.py")
    print(discover)
    return discover
    pass
...
if __name__ == '__main__':
    ...
    report_path = r"E:\pychon-code\a_web_auto_scirpt\report\\" +now + "_RC_103.html"
    # 生成报告的存储路径   并定义报告的名称
    ...
    #发送单个附件
    # yag.send(to='fengxiaodng777@163.com',subject='一家民宿app测试报告',contents,attachments=report_path
    #  被发送的邮箱地址    attachments是发送的附件  本地的路径


    # 给单用户发送邮件并添加多个附件
    yag.send(to="fengxiaodng777@163.com",subject="邮件测试",contents=
    ["测试报告附件中，请查收！！！",r"E:\pychon-code\a_web_auto_scirpt\log\ctrip.txt",report_path],attachments=r'E:\pychon-code\a_web_auto_scirpt\pictures\\*.jpg')
